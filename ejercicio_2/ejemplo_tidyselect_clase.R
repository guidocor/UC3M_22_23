...
data_longer <- data %>% pivot_longer(-c(id) ) 
data_longer


# de la r1 a la r3
data_longer <- data %>% pivot_longer(cols = c(r1,r2,r3))
data_longer

data_longer <- data %>% pivot_longer(cols = r1:r3)
data_longer
...
# que contenga r
data_longer <- data %>% pivot_longer(contains("r"))
data_longer

data_longer <- data %>% pivot_longer(starts_with("r"))
data_longer
...
# las que sean numericas
data_longer <- data %>% pivot_longer(cols = where(is.numeric))
data_longer

ggplot(data_longer, aes(x = name, y = value, color = name)) +
  geom_point(size = 5) + theme_bw() + coord_flip()
