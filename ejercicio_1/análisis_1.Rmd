...
if (!require('pacman')) install.packages('pacman'); library('pacman') #es para que si no está instalado te lo instale, el punto y coma es para que te lo separe en lineas
...
df$e4[df$e4=="Holi"]<-NA
df$e4_num<-as.numeric(df$e4)
class(df$e4_num)

df$e4_q <- (df$e4_num-45)
df$e4_q
...
df$e1_error <- abs(df$e1_q)
df$e2_error <- abs(df$e2_q)
df$e3_error <- abs(df$e3_q)
df$e4_error <- abs(df$e4_q)
df$e5_error <- abs(df$e5_q)
...
```{r}
summary(df$v1_e)
df$a1_hombre<-df$a1_g
df$a1_hombre[df$a1_hombre=="Mujer"]<-0
df$a1_hombre[df$a1_hombre=="Hombre"]<-1
df$a1_hombre[df$a1_hombre=="Prefiero no decirlo"]<-NA
df$a1_hombre<-factor(df$a1_hombre,
                     levels=c(0,1),
                     labels=c("mujer","hombre"))
summary(df$a1_hombre)
summary(df$a2_e)

#install.packages("e1071")
library(e1071)
m1 <- lm(e1_error ~ v1_e+a1_hombre+a2_e, data=df)
summary(m1) 
