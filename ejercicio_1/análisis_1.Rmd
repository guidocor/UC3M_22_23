...
df$e1_error <- (df$e1_q-20)
df$e2_error <- (df$e2_q-25)
df$e3_error <- (df$e3_q-77)
df$e4_error <- (df$e4_q-45)
df$e5_error <- (df$e5_q-70)
...
m1 <- lm(df$e1_error ~ df$v1_e + df$a1_g + df$a2_e, data=df)
summary(m1) 


```

Si no has encontrado una relación significativa puedes explorar con un pequeño gráfico la relación entre la variable v1_e y e1_error para ver hasta qué punto podemos relacionar la seguridad de las evaluaciones con la precisión de las mismas 
...
La fórmula es la siguiente: `rmse = sqrt(mean((valor_real -  valor_predicho)^2))`. La lógica de la misma es queremos que la gente que se equivoque más, tenga más peso (por eso el cuadrado) y obtener una media de cuanto se desvían (de ahí la media) en una escala más entendible (por eso la raíz cuadrada que 'cancela' el cuadrado anterior). Calcula el RMSE de la variable df$e1_error
...
Ahora que conocemos esta lógica podemos crear una función que nos entregue el RMSE de cualquier vector que se le suministre. Crea esta función y aplícala sobre el resto de la variable (df$e2_error, df$e3_error, etc)

```{r}

rmse <- function(x)
{y <- sqrt(mean((x)^2, na.rm = TRUE))
return(y)
}
