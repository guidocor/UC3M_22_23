...
O ¿Cómo podemos saber la media de las distintas evaluaciones de las caras? Hacerlo con el anterior método es difícil. Vamos a probar con alguna función como `janitor::skim()`
...
Ahora queremos observar si en general somos buenos adivinando edades o no. Las edades reales de las personas que han  visto son las siguientes
...
df$e4_q <- as.integer(df$e4_q)
  #se que tengo que eliminar el "holi", porq sino no es numérica, pero no se cómo...
  #tengo que hacer lo mismo q arriba con la función as.integer??
#efectivamente
df$e4_q



df$e1_error <- (20- df$e1_q)
df$e2_error <- (25- df$e2_q)
df$e3_error <- (77- df$e3_q)
df$e4_error <- (45- df$e4_q)
df$e5_error <- (70- df$e5_q)
...
m1 <- lm(df$e1_error~ df$v1_e+df$a1_g+df$a2_e)
summary(m1) 


```

Si no has encontrado una relación significativa puedes explorar con un pequeño gráfico la relación entre la variable v1_e y e1_error para ver hasta que punto podemos relacionar la seguridad de las evaluaciones con la precisión de las mismas 

```{r}
plot(df$v1_e, df$e1_error)#este grafico tiene muy poco sentido 

Cor1<-cor(df$v1_e, df$e1_error)
```
...
rmse = sqrt(mean((df$e1_error)^2))

```

Ahora que conocemos esta lógica podemos crear una función que nos entregue el RMSE de cualquier vector que se le suministre. Crea esta función y aplicala sobre el resto de variables (df$e2_error, df$e3_error, etc)
