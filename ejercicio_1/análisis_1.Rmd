...
```{r}
df <- read.csv("./data/datos_2021.csv",fileEncoding = "UTF-8")
...
class(df$e4_q)
class(df$e5_q)
## Observamos que la df$e4_q es integer y no numerica
df$e4_q
## Observamos que tiene un valor no numerico ("Holi"). Lo convertimos en NA. Creamos otra variable para no perder la original
df$e4_numerica[df$e4=="Holi"]<-NA

df$e4_numerica <- as.integer(df$e4)
df$e4_numerica
class(df$e4_numerica)


df$e1_error <- abs(df$e1_q - 20)
df$e2_error <- abs(df$e2_q - 25)
df$e3_error <- abs(df$e3_q - 77)
df$e4_error <- abs(df$e4_q - 45)
df$e5_error <- abs(df$e5_q - 70)
...
# NO library(e1071)
## NO reg.lineal<- lm(v1_e ~ e1_error, data=df)  
