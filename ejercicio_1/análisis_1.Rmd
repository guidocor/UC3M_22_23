...
```{r}
df <- read.csv("./data/datos_2021.csv", encoding = "UTF")
...
Con estas nuevas variables podemos llevar a cabo una regresión para poder estimar el error cometido (df$e1_error) según las variables v1_e (confianza en la predicción), a1_g (género) y a2_e (edad)


```{r}

m1 <- lm(df$e1_error ~ v1_e + a1_g + a2_e, data=df)
summary(m1) 
